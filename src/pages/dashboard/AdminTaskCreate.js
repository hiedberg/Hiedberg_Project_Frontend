...
// sections
import TaskNewEditForm from '../../sections/@dashboard/admintask/TaskNewEditForm';
...
export default function AdminTaskCreate() {
  ...
  return (
    <Page title="Task: Assign a task to a staff">
      <Container maxWidth={themeStretch ? false : 'lg'}>
        <HeaderBreadcrumbs
          heading={!isEdit ? 'Assign a task to a staff' : 'Assign a task'}
          links={[
            { name: 'Dashboard', href: PATH_DASHBOARD.general.clientApp },
            { name: 'Task', href: PATH_DASHBOARD.clientTask.list },
            { name: !isEdit ? 'Assign task' : currentTask?.id || '' },
          ]}
        />
