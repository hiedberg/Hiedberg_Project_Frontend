...
export default function BookingRoomAvailable() {
  ...
  return (
    <Card>
      <CardHeader title="Pending Tasks" sx={{ mb: 8 }} />
      <ReactApexChart type="radialBar" series={CHART_DATA} options={chartOptions} height={310} />

      <Stack spacing={2} sx={{ p: 5 }}>
        <Legend label="Assigned" number={SOLD_OUT} />
        <Legend label="Pending" number={AVAILABLE} />
      </Stack>
    </Card>
  );
}
...
function Legend({ label, number }) {
  return (
    <Stack direction="row" alignItems="center" justifyContent="space-between">
      ...
      <Typography variant="subtitle1">{number} Tasks</Typography>
    </Stack>
  );
}
